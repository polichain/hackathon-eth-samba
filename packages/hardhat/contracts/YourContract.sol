...
contract Rent {
   ...
   // }
    ...
    function createNewContract(address payable _renter,
     uint _insuranceAmount, uint _rentAmount, uint _definedTime) external {
    ...
    function payRent(uint index) public payable noReentrancy{
       ...
       require(rentContracts[index].state == ContractState.Active,"The contract is not active");
    ...
    function abortContract(uint index) public payable noReentrancy {
        require(rentContracts[index].state == ContractState.Active,"The contract is not active");
        require(rentContracts[index].renter == msg.sender || rentContracts[index].locator == msg.sender , "Only the renter or locator can abort the contract");
          ...
          //  }
            ...
            (bool success, ) = rentContracts[index].renter.call{value: rentContracts[index].rentAmount }("");
    ...
    function payInsurance(uint index) public payable noReentrancy{
        require(rentContracts[index].state == ContractState.StandBy, "Contract is not in stand by");
        require(msg.sender == rentContracts[index].renter && msg.value >= rentContracts[index].insuranceAmount, "Only the renter can do this operation and the insurance must be higher");
    ...
    function receiveInsurance(uint index) internal {
         ...
         (bool success, ) = rentContracts[index].renter.call{value: rentContracts[index].insuranceAmount}("");
          require(success, "transfer failed");
